...
summary: A realtime marketplace for buying and selling small businesses, built in five evenings.
stack:
  - NestJS
  - MongoDB
  - Redis
...
links:
  ...
  - label: source
    to: repos.scoutboard
  - label: companion
    to: repos.httpMonitor
next: enterprise-platform-work
---

<Figure
  src="/images/work/scoutboard/hero.png"
  width="1680"
  height="721"
  breakout
  alt="The ScoutBoard landing page: a dark green hero reading 'Buy a real business with real numbers.', a pill counting 349 live offers on the book, a most-watched card for Ledger Lane Bookkeeping asking $190,000, and a strip below it counting businesses listed, median asking, live offers and median multiple."
  caption="the landing page, with the offer book's live count in the corner"
/>
...
<Figure
  src="/images/work/scoutboard/ai-analyst.png"
  width="1680"
  height="866"
  breakout
  alt="A ScoutBoard listing asking $350,000, its stat tiles and offer history on the left, and on the right an AI analysis panel marking it overpriced: a $250k–$300k fair-value range, a bar putting the asking price above that range, four lines of reasoning, and a suggested opening offer of $275,000."
  caption="the ai analyst's verdict, with a fair-value range and a suggested offer"
/>
...
```ts
// A socket-pushed offer is written straight into the query cache, so the
// realtime update and the fetched data share one source of truth.
socket.on("offer:created", (offer: Offer) => {
  queryClient.setQueryData<Offer[]>(["offers", offer.listingId], (prev = []) =>
...
<Figure
  src="/images/work/scoutboard/listings.png"
  width="1680"
  height="700"
  breakout
  alt="The ScoutBoard marketplace under the heading 'Businesses for sale': category filter chips, a sort dropdown and a cards/rows toggle above a grid of listing cards, each with a photograph, industry and city, yearly revenue, asking multiple, asking price and a live offer count."
  caption="the marketplace, each card a small business for sale"
/>
...
<Callout>
Lesson: invalidate the cache entries you can key precisely; put a short expiry on the ones you can't.
</Callout>
...
- **No accounts.** Anyone can publish a listing or post an offer, so there is no seller side: no "my listings", no accepting or declining an offer, no way to tell two buyers apart beyond the name they typed.
